...
try {
  ...
  console.log('[Sheets Config]', { client_email: sa.client_email, sheet: process.env.GOOGLE_SHEETS_ID });
} catch(e) { console.log('SERVICE ACCOUNT JSON parse error'); }

async function loadDirectories() {
  const [teachers, students, courses, events] = await Promise.all([
    sheets.read('Teacher Directory'),
    sheets.read('Student Directory'),
    sheets.read('Course List'),
    sheets.read('Event')
  ]);
  return { teachers, students, courses, events };
}
...
app.post('/api/auth/student/login', async (req, res) => {
  ...
  const passRows = await sheets.read('Pass');
  const row = passRows.find(r => (r.role||'').trim().toLowerCase()==='student' && (r.id||'').trim()===student_id);
  const ok = row ? await argon2.verify((row.hash||'').trim(), password).catch(()=>false) : false;
...
app.post('/api/auth/teacher/login', async (req, res) => {
  ...
  const { teachers } = await loadDirectories();
  const exists = teachers.find(t => t.teacher_id === teacher_id);
  if (!exists) return res.json({ ok: false, code: 'AUTH_FAILED', message: 'ID/パスが不正' });

  const passRows = await sheets.read('Pass');
  const row = passRows.find(r => (r.role||'').trim().toLowerCase()==='teacher' && (r.id||'').trim()===teacher_id);
  const ok = row ? await argon2.verify((row.hash||'').trim(), password).catch(()=>false) : false;
  if (!ok) return res.json({ ok: false, code: 'AUTH_FAILED', message: 'ID/パスが不正' });
